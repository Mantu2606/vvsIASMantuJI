...
namespace FossTech.Areas.FossTech.Controllers
{
    ...
    public class LatestNewsController : Controller
    {
        ...
        {
            var results = await _context.LatestNews.ToListAsync();
            return new ExcelResult<LatestNews>(results, "Demo Sheet Name", "Fingers10");
        }
        ...
        public async Task<IActionResult> Create(IFormFile formImage, LatestNews post)
        {
            ViewData["Sections"] = new MultiSelectList(_context.Sections, "Id", "Name");
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var post = await _context.LatestNews.FindAsync(id);
            if (post == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, IFormFile image, LatestNews post)
        {
            ViewData["Sections"] = new MultiSelectList(_context.Sections, "Id", "Name");
            if (id != post.Id)
            {
            ...
            var thisPost = await _context.LatestNews.FirstAsync(p => p.Id == post.Id);
        ...
        {
            var post = await _context.LatestNews.FindAsync(id);
            _context.LatestNews.Remove(post);
            await _context.SaveChangesAsync();
            return RedirectToAction(nameof(Index));
        }
        ...
        public async Task<IActionResult> DeleteSelected(List<int> leadIds)
        {
            ...
            var leadsToDelete = await _context.LatestNews
            .Where(lead => leadIds.Contains(lead.Id))
            .ToListAsync();

            _context.LatestNews.RemoveRange(leadsToDelete);
            await _context.SaveChangesAsync();
